...
MusicVisualizer.prototype.ini = function(container, canvasId, volumnId){
	var self = this;
	this.box = container;
	this.cxt = document.getCSSCanvasContext("2d", canvasId, this.box.clientWidth, this.box.clientHeight); //将canvas用作背景
...
MusicVisualizer.prototype.decode = function(url, arraybuffer, cb){
	...
	MusicVisualizer.ac.decodeAudioData(arraybuffer, function(buffer){
		if(self.source.curr){
			self.source.bs && self.source.bs[self.source.bs.stop?"stop":"nodeOff"]();
		}
...
MusicVisualizer.prototype.play = function(url, isMobile/*是否移动设备*/){
	...
	if(!isMobile){
		...
		self.source.bs = self.audioSource;
		self.source.bs.connect(self.gainNode);
		//MusicVisualizer.play1(self);
		//return;
	}else{
...
MusicVisualizer.prototype.visualizer = function(){
	...
	function v(){
		...
		//console.log(arr);
		//self.drawCanvasDot(arr);
		self.drawCanvasRect(arr);
